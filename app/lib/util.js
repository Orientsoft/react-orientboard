...
export function startDynamic(component) {
  ...
  const cb = component.handleDynamicData ? (data) => {
  ...
  const serverURL = component.state.datasource.server
...
export function stopDynamic(component) {
  const cb = component.handleDynamicData ? component.handleDynamicData : (data) => { console.log(data) }
